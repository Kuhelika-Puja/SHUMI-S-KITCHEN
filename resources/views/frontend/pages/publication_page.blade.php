  ...
  <div class="container">
    <div class="row">
      ...
      <div class="col-md-4 py-3">
        <div class="">
          <?php
          if(isset($publication->image) && !empty($publication->image)){
          ?>
          <a href="{{url('publication/id',$publication->id)}}" class="font-weight-bold">
            <img src="{{url($publication->image)}}" class="card-img-top" alt="..." style="height: 300px;width: 100%;object-fit: cover;object-position: top;">
          </a>
          <?php
          }else{
          ?>
          ...
          <div class="card-body custom-card-body text-center">
            <p class="card-title mp-0">{{$publication->title}}</p>
            <p class="mp-0"><?php //echo $blog->created_at->diffForHumans();?> <?php echo date('d M Y', strtotime($publication->created_at)); ?></p>
          </div>
        </div>
      </div>
      @endforeach
